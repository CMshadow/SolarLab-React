...
let request_board_working_condition_center = (store) => {
    ...
    fetch('/mock/Sample Array Pmp, Sample Array Imp, Sample Array Vmp (Board Working Condition).json')
        ...
        .then(response_json => {
            ...
            store.dispatch({ type: 'SUCCESS_BOARD_WORKING_CONDITION_CENTER', payload: load_option, displayMode: displayMode });
        })
        .catch(error => {
            // error handler
...
let reload_board_working_condition_center = (store, displayMode) => {
    store.dispatch({ type: 'RELOAD_BOARD_WORKING_CONDITION_CENTER', displayMode: displayMode });
    request_board_working_condition_center(store);
};


export { request_board_working_condition_center, reload_board_working_condition_center }
